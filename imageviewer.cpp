...
void ImageViewer::setImage(const QImage &newImage)
{
    ...
    if (image.colorSpace().isValid())
        image.convertedToColorSpace(QColorSpace::SRgb);
...
static void initializeImageFileDialog(QFileDialog &dialog, QFileDialog::AcceptMode acceptMode)
{
    ...
    for (const QByteArray &mimeTypeName : supportedMimeTypes)
        mimeTypesFilters.append(mimeTypeName);
    mimeTypesFilters.sort();
    dialog.setMimeTypeFilters(mimeTypesFilters);
...
{
    bool fitToWidth = fitToWidthAct->isChecked();
    if (fitToWidth) {
...
{
    bool fitToHeight = fitToHeightAct->isChecked();
    if (fitToHeight) {
        scaleImage((double)scrollArea->height() / (double)imageLabel->height());
    } else {
        normalSize();
    }
    updateActions();
...
{
    QMessageBox::about(this, tr("About Quinoa Image Viewer"),
                       tr("<p>The <b>Quinoa Image Viewer</b> is an image viewer</p>"));
}
...
void ImageViewer::createActions()
{
    ...
    fitToWidthAct = viewMenu->addAction(tr("&Fit to Width"), this, &ImageViewer::fitToWidth);
    fitToWidthAct->setEnabled(false);
    fitToWidthAct->setCheckable(true);
    fitToWidthAct->setShortcut(tr("Ctrl+W"));

    fitToHeightAct = viewMenu->addAction(tr("&Fit to Height"), this, &ImageViewer::fitToHeight);
    fitToHeightAct->setEnabled(false);
    fitToHeightAct->setCheckable(true);
    fitToHeightAct->setShortcut(tr("Ctrl+H"));
...
void ImageViewer::updateActions()
{
    ...
    normalSizeAct->setEnabled(!fitToWindowAct->isChecked() && !fitToWidthAct->isChecked() && !fitToHeightAct->isChecked());
    fitToWidthAct->setEnabled(!fitToWindowAct->isChecked() && !fitToHeightAct->isChecked());
    fitToHeightAct->setEnabled(!fitToWindowAct->isChecked() && !fitToWidthAct->isChecked());
